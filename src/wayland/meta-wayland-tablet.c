...
#include "tablet-server-protocol.h"
                                           ...
                                           struct wl_resource *resource)
{
  ...
  zwp_tablet1_send_name (resource, clutter_input_device_get_device_name (device));

  if (sscanf (clutter_input_device_get_vendor_id (device), "%x", &vid) == 1 &&
      sscanf (clutter_input_device_get_product_id (device), "%x", &pid) == 1)
    zwp_tablet1_send_id (resource, vid, pid);

  /* FIXME: zwp_tablet1.type, zwp_tablet1.path missing */

  zwp_tablet1_send_done (resource);
}
                                         ...
                                         uint32_t            id)
{
  ...
  resource = wl_resource_create (client, &zwp_tablet1_interface,
