      ...
      if (!wl_list_empty (l))
        {
          ...
          wl_resource_for_each (resource, l)
            {
              zwp_tablet_tool1_send_proximity_in (resource, tool->proximity_serial,
...
static const struct zwp_tablet_tool1_interface tool_interface = {
  tool_set_cursor,
  tool_destroy
};
                                              ...
                                              uint32_t               id)
{
  ...
  resource = wl_resource_create (client, &zwp_tablet_tool1_interface,
